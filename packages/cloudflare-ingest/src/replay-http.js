...
export async function handlePostHogReplay(request, env, options = {}) {
  ...
  if (request.headers.get("authorization") !== `Bearer ${env.ETLAYER_REPLAY_KEY}`) {
  ...
  try {
    const replay = options.replay || replayPostHogRange;
    ...
    return jsonResponse(
      {
        error: error instanceof Error ? error.message : "replay delivery failed",
      },
      502,
    );
  }
